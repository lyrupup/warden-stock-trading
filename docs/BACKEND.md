...
users 1──N strategies 1──N strategy_indicators       (量化因子规则组 JSONB)
                     1──1 strategy_skills (含版本)
                     1──N strategy_screen_results     (量化粗筛任务/结果, 候选池)
...
公共:  stock_indicator_snapshots (个股技术指标快照, 可选, 全市场粗筛加速)
```
...
-- 指标定义（量化因子规则组 JSONB）
-- conditions 形如 {logic:"and"|"or", rules:[{left,op,right}...], groups:[<同结构嵌套>...]}
-- left/right 为因子表达式 {type,...params} 或常量 {type:"const",value}；详见 §5 M2 因子引擎
...
-- M2 量化粗筛任务/结果（异步，复用 backtest 同款状态机）
CREATE TABLE IF NOT EXISTS strategy_screen_results (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL,
    strategy_id BIGINT NOT NULL,
    status SMALLINT NOT NULL DEFAULT 0,      -- 0 pending 1 running 2 done 3 failed
    params JSONB NOT NULL DEFAULT '{}',       -- {universe:{type,codes,board}, trade_date, limit}
    universe_count INT NOT NULL DEFAULT 0,    -- 实际扫描标的数
    matched_count INT NOT NULL DEFAULT 0,     -- 命中候选数
    candidates JSONB,                         -- [{stock_code,stock_name,score,factors:{ma5,ma10,...,bias5,amplitude},matched:[ruleIdx...]}]
    trade_date DATE,                          -- 粗筛基准交易日
    error_msg VARCHAR(512) NOT NULL DEFAULT '',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- M2 个股技术指标快照（公共, 可选）：由 M5 盘后定时任务批量预计算，
-- 全市场粗筛时可直接走 SQL 过滤，避免每次实时拉 K 线重算（性能优化，非 V1 必需）。
CREATE TABLE IF NOT EXISTS stock_indicator_snapshots (
    ...
    amplitude NUMERIC(10,4),                  -- 当日振幅%
    amplitude_streak SMALLINT NOT NULL DEFAULT 0, -- 截至当日连续振幅>=阈值天数(按默认阈值预算, 仅参考)
    turnover_rate NUMERIC(10,4), volume NUMERIC(20,0), change_percent NUMERIC(10,4),
    factors JSONB,                            -- 其余扩展因子键值, 便于新增因子免改表
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
| POST | `/strategies/:id/backtest` | 发起回测 → `{taskId}`（后续迭代） |
| GET | `/strategies/:id/backtest/:taskId` | 查询回测结果（后续迭代） |
| GET | `/strategies/indicators/catalog` | **量化因子目录**（可选因子类型/参数/说明，供前端构造器渲染） |
| GET | `/strategies/templates` | **内置策略模板**（短线多头 / 中长线震荡 等，含量化指标定义） |
| POST | `/strategies/:id/screen` | **发起量化粗筛（异步）** → `{taskId}` |
| GET | `/strategies/:id/screen/:taskId` | 查询粗筛结果（候选列表 + 指标快照） |
| GET | `/strategies/:id/screen/latest` | 最近一次粗筛结果 |
| POST | `/strategies/screen/preview` | **同步快速粗筛预览**（小股票池/自选池，便于调参，无需建任务） |

请求示例（新建「短线均线多头」策略，量化因子规则组）：

```json
POST /api/strategies
{
  "name": "短线均线多头排列",
  "description": "MA5>MA10>MA20 主升浪追高票",
  "tags": "短线,趋势,主升浪",
  "indicators": { "logic": "and", "rules": [
    {"left":{"type":"ma_align","periods":[5,10,20],"direction":"bull"},"op":"==","right":{"type":"const","value":true}},
    {"left":{"type":"field","field":"close"},"op":">","right":{"type":"ma","period":5}},
    {"left":{"type":"bias","period":5},"op":"<=","right":{"type":"const","value":8}}
  ]}
}
```
...
```json
POST /api/strategies/12/screen
{
  "universe": { "type": "all" },          // all 全市场 | board 指定板块 | watchlist 自选股 | codes 自定义
  "board": "",                             // type=board 时给板块代码
  "codes": [],                             // type=codes 时给代码列表
  "trade_date": "",                        // 留空=最新交易日
  "limit": 200                             // 候选上限（按评分截断）
}
// → { "code":0, "data": { "taskId": "37" } }
```

粗筛结果（候选每只携带命中因子快照值，便于前端展示与 AI 交接）：

```json
GET /api/strategies/12/screen/37
{ "code":0, "data": {
  "id":37, "status":2, "trade_date":"2026-05-29",
  "universe_count":4832, "matched_count":18,
  "candidates":[
    {"stock_code":"600519","stock_name":"贵州茅台","score":"0.87",
     "factors":{"ma5":"1710.20","ma10":"1685.30","ma20":"1650.10","bias5":"3.21","amplitude":"4.85"},
     "matched":[0,1,2]}
  ]
}}
```

> **粗筛 → AI 精筛交接**：候选个股可直接调用 M6 `POST /ai/conversations`（携带 `strategy_id + stock_code`）发起分析，AI 加载该策略 skill.md 做精筛，无需在 M2 重复实现 AI 逻辑。
...
- 指标条件以 JSONB（`{logic, rules[], groups[]}`）存储，**粗筛引擎**与回测引擎共用同一套规则解析。
- skill.md 每次保存 `version+1`，保留历史；AI 分析按最新版本加载。
- **回测异步化（后续迭代）**：`POST backtest` 创建 `backtest_results(status=pending)` → 投递 MQ/启 goroutine → 引擎用历史行情逐日撮合 → 写回 `metrics/curve/status`。Service 层用 `select{case <-ctx.Done()}` 支持中断。

#### M2 量化指标粗筛（本期核心）

把盘面经验量化为**可计算因子 + 可组合规则**，由后台对股票池做纯规则粗筛，产出候选池，作为后续 M6 AI 精筛的输入。整体为「**因子引擎 → 规则引擎 → 粗筛编排**」三层，全部为纯函数 / 可注入接口，便于 TDD。

**① 因子引擎（`internal/strategy/factor`）**

输入是一只股票按时间升序的前复权日 K 线序列（`[]model.Kline`，最后一根为当前 bar）；每个因子产出一个数值或布尔值。因子用注册表按 `type` 分发，新增因子只需实现接口并注册（开闭原则，对应 PRD「通用因子库」）。

```go
// internal/strategy/factor/factor.go
package factor
...
type Series struct{ Bars []model.Kline }

// Expr 因子表达式：与 JSONB 中 left/right 对齐。
type Expr struct {
    Type      string  `json:"type"`                 // ma|ma_align|bias|amplitude|amplitude_streak|pct_change|field|vol_ratio|const
    ...
    Value     any     `json:"value,omitempty"`      // const: number|bool
}

// Eval 计算因子值；数据不足（如 K 线不够算 MA60）返回 ErrInsufficientData。
func Eval(e Expr, s Series) (Value, error) { /* 按 type 分发 */ }
```

**② 量化公式（务必单测覆盖，对应 PRD 指标量化口径）**

- `MA(N)`：`sum(close[t-N+1..t]) / N`，K 线不足 N 根报 `ErrInsufficientData`。
- `ma_align`（多头排列）：对有序周期 `[p1<...]` 校验 `MA(p1) > MA(p2)+eps > ...`；`bull` 严格递减、`bear` 严格递增。
- `bias`（乖离率）：`(close_t - MA(N)) / MA(N) * 100`，`MA(N)==0` 防除零。
- `amplitude`（振幅）：`(high_t - low_t) / prev_close_t * 100`，`prev_close_t==0` 防除零。
- `amplitude_streak`（连续振幅）：从 `t` 向前累计 `amplitude(bar) >= threshold` 的连续根数，`>= days` 为真。
- `pct_change(N)`：`(close_t - close_{t-N}) / close_{t-N} * 100`。

**③ 规则引擎（`internal/strategy/rule`）**

```go
type RuleGroup struct {
    Logic  string      `json:"logic"`            // and|or
    Rules  []Rule      `json:"rules"`
    Groups []RuleGroup `json:"groups,omitempty"` // 嵌套
}
type Rule struct {
    Left  factor.Expr `json:"left"`
    Op    string      `json:"op"`                // > >= < <= == !=
    Right factor.Expr `json:"right"`             // 因子或 {type:"const",value}
}

// Eval 对一只股票执行规则组，返回是否命中 + 命中的规则索引（用于前端高亮/评分）。
func Eval(g RuleGroup, s factor.Series) (matched bool, hitIdx []int, snap map[string]factor.Value, err error)
```

- 比较时左右各 `factor.Eval`；布尔因子与常量布尔比较，数值因子用 `decimal` 比较。
- `and` 全真才命中、`or` 任一真即命中，组可递归嵌套。
- `snap` 收集本次用到的因子快照值，写入候选 `factors`，供前端展示与 AI prompt 复用。
- **缺数据策略**：某因子 `ErrInsufficientData` 视为该规则不命中（不报错中断整轮粗筛），并在结果里标注剔除原因（可选）。

**④ 粗筛编排（`internal/service/screen.go`，异步，复用 backtest 状态机）**

```
POST /strategies/:id/screen
  → 校验策略归属 → 建 strategy_screen_results(status=pending) → 返回 taskId
  → goroutine（带 ctx 可中断）：
      1. 解析 universe（all/board/watchlist/codes）→ 标的代码列表
      2. 并发分批（worker pool, 受 gotdx 连接池约束）拉每只前复权日 K 线（足够根数, 如 ≥ max(periods)+streak+buffer）
         - 优先读 Redis 缓存 warden:kline:{code}:day:qfq；命中走 stock_indicator_snapshots（若已预算）更快
      3. 对每只 rule.Eval → 命中则收集 {code,name,score,factors,matched}
      4. 评分 score：可按乖离适中度/振幅强度/排列斜率加权（默认=命中规则占比），按分降序、limit 截断
      5. 写回 candidates/universe_count/matched_count/status=done；异常 status=failed,error_msg
...
- **同步预览** `POST /strategies/screen/preview`：限定小池（如自选股 ≤ N 只）即时返回，便于用户在指标构造器里边调边看，不落任务表。
- 因子目录 `GET /strategies/indicators/catalog` 与模板 `GET /strategies/templates` 返回**内置只读元数据**（因子 type/参数 schema/说明、两个预置策略模板），保证前端构造器与后端引擎口径一致（单一事实源）。

**建议错误码（21xxx 策略段）**：`21001 策略不存在`、`21002 指标规则非法（未知因子/缺参数/op 不支持）`、`21003 股票池为空或过大`、`21004 粗筛任务不存在`、`21005 粗筛执行失败`、`21006 K 线数据不足以计算指标`。
...
| M2 策略 | **因子计算 `factor.Eval`**、**规则引擎 `rule.Eval`**、粗筛编排、复权计算 `applyAdjust` | MA/排列(bull/bear)/乖离/振幅/连续振幅口径正确、K 线不足报 `ErrInsufficientData`、因子间比较(MA5>MA10)、and/or 嵌套、布尔因子与常量比较、缺数据不中断整轮、universe 解析、limit 截断、粗筛 ctx 中断、前/后/不复权口径正确 |
